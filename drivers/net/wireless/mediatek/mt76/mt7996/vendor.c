				       ...
				       int data_len)
{
	...
	if (!band_idx_get && (tb[MTK_VENDOR_ATTR_WIRELESS_CTRL_FIXED_OFDMA] ||
	    tb[MTK_VENDOR_ATTR_WIRELESS_CTRL_NUSERS_OFDMA] ||
	    tb[MTK_VENDOR_ATTR_WIRELESS_CTRL_MIMO] ||
	    tb[MTK_VENDOR_ATTR_WIRELESS_CTRL_CERT]))
		return -EINVAL;

	if (tb[MTK_VENDOR_ATTR_WIRELESS_CTRL_FIXED_OFDMA]) {
		val8 = nla_get_u8(tb[MTK_VENDOR_ATTR_WIRELESS_CTRL_FIXED_OFDMA]);
